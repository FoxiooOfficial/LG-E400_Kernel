...
struct timezone
{
	...
	#define TIME64_MAX		((time64_t)((1ULL << 63) - 1))
	...
	*/
	...
	static inline int timeval64_compare(const struct timeval64 *lhs, const struct timeval64 *rhs)
	{
		if (lhs->tv_sec < rhs->tv_sec)
			return -1;
	...
	*/
	...
	extern struct timespec64 timespec64_add_safe(const struct timespec64 lhs, const struct timespec64 rhs);

	static inline struct timespec timespec_add(struct timespec lhs, struct timespec rhs)
	{
		struct timespec ts_delta;

		set_normalized_timespec(&ts_delta, 
	...
	static inline struct timespec64 timespec64_add(struct timespec64 lhs, struct timespec64 rhs)
	{
		...
		set_normalized_timespec64(&ts_delta, 
								lhs.tv_sec + rhs.tv_sec,
								lhs.tv_nsec + rhs.tv_nsec);
